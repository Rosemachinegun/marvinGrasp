...
可直接更新 Conda 环境：
conda activate flowpose
conda env update -f environment-flowpose.yml
或者在现有环境里单独安装：
conda install -c conda-forge portaudio
pip install "gradio>=4.44,<6" "sounddevice>=0.4" "faster-whisper>=1.0"

运行前建议检查：
python -c "import gradio, sounddevice, faster_whisper; print('OK')"
python -c "import sounddevice as sd; print(sd.query_devices())"

配置方面：
- UI 默认监听 0.0.0.0:7860
- 平板访问：http://<机器人IP>:7860
- 如果端口被占用，可使用：
python flowpose_request_ik_tester.py --tablet-ui-port 7861
- 分别关闭功能：
