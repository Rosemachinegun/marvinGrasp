...
# ``httpx`` (used by faster-whisper/huggingface_hub) does not accept the
# generic ``socks://`` scheme exported by some desktop proxy clients.  Remove
# only invalid SOCKS values before the Whisper model is created; valid HTTP(S)
# proxy settings remain available for model downloads.
for _proxy_var in (
    "HTTP_PROXY",
    "HTTPS_PROXY",
    "ALL_PROXY",
...
class VoiceToText:
    ...
    def record(
        ...
        # Many USB/ALSA microphones reject 16 kHz even though Whisper expects
        # 16 kHz input. Record at the hardware's native rate first, then
        # resample below.
        native_rate = self.check_microphone()

        frame_count = int(
            round(seconds * native_rate)
        )
        ...
        if native_rate != self.config.sample_rate and audio.size:
            ...
            target_length = int(
                round(audio.size * self.config.sample_rate / native_rate)
            )
