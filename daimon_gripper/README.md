...
`daimon_gripper` 收集了 Daimon 夹爪、远程相机和触觉检测相关的 Python 工具。这个目录主要用于本地调试、双夹爪控制、双相机预览，以及触觉/滑移检测实验。
...
```bash
cd daimon_gripper/dm_gripper_cam_py
...
```bash
cd daimon_gripper/dm_gripper_tac_py
python -m pip install .
```
