...
from grasp_core.core.math.easing import smoothstep
...
def apply_downward_end_effector_tilt(
    ...
    hand_name = "left" if str(hand).strip().lower() == "left" else "right"
    xyz_values = getattr(args, f"grasp_orientation_xyz_rad_{hand_name}", None)
    if xyz_values is not None:
        xyz_rotation = rotation_matrix_from_xyz_euler_rad(tuple(xyz_values))
        tilted_pose = np.asarray(pose, dtype=np.float64).copy()
        if getattr(args, "ik_downward_tilt_frame", "local") == "base":
            tilted_pose[:3, :3] = xyz_rotation @ tilted_pose[:3, :3]
        else:
            tilted_pose[:3, :3] = tilted_pose[:3, :3] @ xyz_rotation
        return tilted_pose
...
def rotation_matrix_from_xyz_euler_rad(
    orientation_xyz_rad: tuple[float, float, float],
) -> np.ndarray:
    """Return ``Rz(z) @ Ry(y) @ Rx(x)`` for XYZ roll/pitch/yaw radians."""
    roll, pitch, yaw = (float(value) for value in orientation_xyz_rad)
    cr, sr = float(np.cos(roll)), float(np.sin(roll))
    cp, sp = float(np.cos(pitch)), float(np.sin(pitch))
    cy, sy = float(np.cos(yaw)), float(np.sin(yaw))
...
def select_ik_hand(position_xyz: np.ndarray, hand_mode: str) -> str:
    if hand_mode in {"left", "right"}:
        return hand_mode
    return "left" if float(position_xyz[1]) > 0.1 else "right"
