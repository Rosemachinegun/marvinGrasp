...
from grasp_core.execution.motion_executor import (  # noqa: E402
    build_ik_target_publisher,
    request_ik_publisher_unavailable_status,
)
from grasp_core.tools.request_ik_ui import make_dashboard  # noqa: E402
from grasp_core.core.types.robot_target_pose import TargetObjectPose  # noqa: E402
from grasp_core.core.io.target_pose_loader import (  # noqa: E402
    load_camera_extrinsic_from_xacro,
)
from grasp_core.core.math.pose import select_ik_hand  # noqa: E402
from grasp_core.execution.robot_skill_service import (  # noqa: E402
    RobotActionService,
    action_failed,
    grip_confirmed,
...
from grasp_core.apps.voice_grasp import VoiceGraspInput  # noqa: E402
...
class RuntimeState:
    ...
    # A successful place intentionally leaves this hand at the place pose. It
    # stays there for the next grasp and only returns HOME when no targets remain.
    parked_after_place_hand: str | None = None
...
class GraspDemoApp:
    ...
    def __init__(self, args: Any) -> None:
        # Grasp owns its motion configuration.  Keep external args for runtime
        # dependencies, while preventing CLI/YAML grasp values from changing
        # the actual grasp behavior.
        args = GRASP_PLANNER.controlled_args(args)
        self.args = args
        self.state = RuntimeState()
        self.gripper_future: Future | None = None
        self.gripper_future_command: str | None = None
        self.gripper_future_hand: str | None = None
        self.gripper_future_followup = True
        self.grasp_future: Future | None = None
        self.place_future: Future | None = None
        ...
        self.voice = VoiceGraspInput(
            enabled=bool(getattr(args, "voice_input", False)),
        )
        ...
        # All robot motions share one ROS publisher and must be serialized.
    ...
    def request_voice_command(self) -> None:
        """Start recognition while the robot workflow is idle."""
        status = self.voice.request(robot_busy=self.voice_busy())
    ...
    def voice_busy(self) -> bool:
        ...
        return (
            s.paused
            or bool(voice is not None and voice.task_active)
            ...
            or any(is_pending(future) for future in (
                s.sam_future,
                s.flowpose_future,
                self.grasp_future,
                self.place_future,
                self.gripper_future,
                self.manual_home_future,
            ))
    ...
    def handle_voice_commands(self) -> None:
        """Consume speech results on the camera/robot owner thread."""
        while True:
            command = self.voice.poll()
            if command is None:
                return
            if self.voice_busy():
                print(f"[voice] busy; command discarded: {command.text}", flush=True)
                continue
            if command.target is None:
                self.state.status = command.status
                print(f"[voice] {command.status}", flush=True)
                continue
            if not bool(getattr(self.args, "enable_place_after_grasp", True)):
                self.state.status = "Voice grasp requires place-after-grasp enabled"
                print(f"[voice] {self.state.status}", flush=True)
                continue
            self.voice.activate(command.target)
            if self.start_pipeline(grasp_on_done=True):
                print(
                    f"[voice] {command.text} -> {command.target}; grasp started",
                    flush=True,
                )
            else:
                self.voice.finish_task()
    ...
    def sam_prompt_for_current_task(self) -> str:
        ...
        return voice.sam_prompt(
            self.args.prompts,
            getattr(self, "pick_templates", None),
        )
    ...
    def _collect_gripper_result(self) -> None:
        ...
        if getattr(self, "gripper_interrupted", False):
            # A grip completed during/after S must not trigger place or recovery.
        ...
        followup = getattr(self, "gripper_future_followup", True)
        if not followup:
            self.gripper_future = None
            self.gripper_future_command = None
            self.gripper_future_hand = None
        ...
        if command == "grip":
            ...
            if (
                ...
                self.auto_place_after_confirmed_grasp()
            else:
                self.state.grasp_confirmed = False
                self.state.grasp_confirmed_hand = None
                self.state.grasp_confirmed_label = None
    ...
    def send_gripper(
        ...
        *,
        ...
        self.gripper_future_followup = followup
    ...
    def publish_grasp(self) -> None:
        ...
        if self.grasp_future is not None and not self.grasp_future.done():
            ...
            return
        if is_pending(self.place_future):
            self.state.status = "Grasp waiting for place action"
            return
        if self.ik_publisher is not None:
    ...
    def home_both_when_no_targets(self) -> None:
        ...
        if self.state.parked_after_place_hand not in {"left", "right"}:
            return
        if self.manual_home_future is not None:
            return
        self.state.home_needs_sync.update({"left", "right"})
        self.state.parked_after_place_hand = None
        self.manual_home_hand = ("left", "right")
        self.manual_home_future = self.action_executor.submit(
            self._publish_manual_homes,
            ("left", "right"),
            frozenset(self.state.home_needs_sync),
        )
        self.state.status = "No targets remain: both hands HOME"
    ...
    def finish_grasp_result(self, result) -> None:
        ...
        self._grip_return_at = time.monotonic()
    ...
    def auto_place_after_confirmed_grasp(self) -> None:
        """Run fixed place immediately after a confirmed successful grasp."""
        if not bool(getattr(self.args, "enable_place_after_grasp", True)):
            return
        self.start_place()

    def start_place(self) -> None:
        """Schedule Place without blocking the camera/UI loop."""
        if self.place_future is not None and not self.place_future.done():
            self.state.status = "Place action already running"
            return
        if self.robot_actions is None:
            self.state.status = "Robot action service unavailable"
            return
        self.place_target_count = len(self.state.base_targets)
        ...
        self.place_future = self.action_executor.submit(
            ...
            target_count=self.place_target_count,
        )
        self.state.status = "Place action running"

    def collect_place_result(self) -> None:
        """Consume the asynchronous Place result and advance the workflow."""
        if self.place_future is None or not self.place_future.done():
            return
        try:
            result = self.place_future.result()
        except Exception as exc:  # noqa: BLE001
            self.state.status = f"Auto place failed: {exc}"
            print(f"[place] {self.state.status}", flush=True)
            self.place_future = None
            self.place_target_count = None
            self.finish_voice_task()
            return
        target_count = self.place_target_count
        self.place_future = None
    ...
    def publish_place(self) -> None:
        """Backward-compatible entry point for scheduling Place."""
        self.start_place()

    def restart_grasp_pipeline_after_place(self) -> None:
        """Optionally repeat the same perception-and-grasp workflow as the A key."""
        voice = getattr(self, "voice", None)
        if voice is not None and voice.task_active:
            print(f"[voice] one-shot {voice.active_target} complete", flush=True)
            self.finish_voice_task()
            return
        if not bool(getattr(self.args, "continuous_grasp_after_place", True)):
            print("[place] continuous grasp disabled", flush=True)
            return
        print("[place] place complete; restarting A-key grasp pipeline", flush=True)
    ...
    def advance_continuous_grasp(self) -> None:
        """Retry a post-place pipeline start once transient blockers are gone."""
    ...
    def start_drop_recovery(self, status: str, hand: str | None) -> None:
        ...
        if self.ik_publisher is None or self.robot_actions is None:
            s.status = (
                "Drop recovery failed; "
                f"{request_ik_publisher_unavailable_status(self.args)}; "
                "remaining paused"
            )
            self.finish_voice_task()
            return
        generation = self.ik_publisher.request_stop()
        # Place has returned before this method is called. Drain any other old
    ...
    def start_grip_failure_recovery(
        ...
        *,
        ...
        """Stop old motion, release, settle, then recover from measured FK."""
        s = self.state
        s.pipeline_stage = PipelineStage.IDLE
        ...
        s.retry_attempts += 1
        # A failed grasp is retried without an attempt limit.  Keep the
        # explicit retry-loop switch as the operator's safety stop.
        retry_enabled = bool(getattr(self.args, "grip_retry_loop", True))
        s.retry_will_regrasp = retry_enabled
        ...
        if self.robot_actions is None or self.ik_publisher is None:
            s.status = "Grip failure recovery unavailable; remaining stopped"
            s.retry_will_regrasp = False
            self.finish_voice_task()
            return
        ...
        previous_actions = tuple(
            future
            for future in (
                getattr(self, "grasp_future", None),
                getattr(self, "place_future", None),
                getattr(self, "manual_home_future", None),
                getattr(self, "gripper_future", None),
            )
        ...
        retry_text = (
            f"retry {s.retry_attempts} (unlimited)"
            if s.retry_will_regrasp
            else "retry loop disabled"
        )
        s.status = (
            f"Grip failed; stopping old motion, releasing {hand}, then moving "
            f"from measured FK to recovery; {retry_text}"
        )
        print(f"[grip_retry] {s.status}", flush=True)

    def _recover_failed_grasp(
        self,
        hand: str,
        generation: int,
        previous_actions: tuple[Future, ...],
    ) -> str:
        """Run failed-grasp recovery in a strict, race-free sequence."""

        for future in previous_actions:
            try:
                future.result()
            except Exception as exc:  # noqa: BLE001
                print(f"[grip_retry] interrupted action exited: {exc}", flush=True)
        if self.ik_publisher.stop_generation() != generation:
            raise RuntimeError("grip recovery cancelled by a newer stop request")

        print(f"[grip_retry] old motion stopped; releasing {hand}", flush=True)
        release_status = self.robot_actions.send_gripper("release", hand)
        if action_failed(release_status):
            raise RuntimeError(f"grip recovery release failed: {release_status}")

        print(
            f"[grip_retry] {hand} released; waiting for settled measured FK",
            flush=True,
        )
        return self.robot_actions.publish_failure_recovery(
            hand,
            fresh_measured_start=True,
            resume_stop_generation=generation,
        )
    ...
    def _collect_recovery_results(self) -> bool:
        ...
        for future in s.recovery_futures:
            ...
            try:
                ...
                s.recovery_errors.append(str(exc))
    ...
    def update_recovery(self) -> None:
        ...
        if s.recovery_errors:
            ...
            if self.ik_publisher is not None:
                self.ik_publisher.request_stop()
            s.status = (
                "Grip recovery failed; automatic regrasp disabled; remaining "
                "stopped: " + "; ".join(s.recovery_errors)
            )
            print(f"[grip_retry] {s.status}", flush=True)
            self.finish_voice_task()
            return
        ...
        if s.replan_pending:
            ...
            s.status = "Recovery done; grip retry loop disabled"
            self.finish_voice_task()
    ...
    def start_pipeline(self, *, grasp_on_done: bool = True) -> bool:
        ...
        if (
            ...
            or is_pending(self.place_future)
        ...
        if s.sam_future is None:
            ...
            return False
        # A manual A-key start also satisfies any queued post-place restart, so it
    ...
    def advance_pipeline(self) -> None:
        ...
        if s.pipeline_stage is PipelineStage.FLOWPOSE:
            ...
            if not s.base_targets:
                ...
                s.pipeline_grasp_on_done = True
                self.home_both_when_no_targets()
                self.finish_voice_task()
                return
    ...
    def render(self, bundle) -> None:
        ...
        dashboard = make_dashboard(
            ...
            place_enabled=bool(getattr(self.args, "enable_place_after_grasp", True)),
        )
    ...
    def _publish_manual_homes(
        self,
        hands: tuple[str, ...],
        sync_hands: frozenset[str],
    ) -> str:
        # The ROS publisher/client is not safe to drive concurrently: its
        # executor-backed generator can raise "generator already executing".
        # Keep both HOME commands in one worker and publish them serially.
        results = []
        for hand in hands:
            status = self.robot_actions.publish_home(
                hand,
                fresh_measured_start=hand in sync_hands,
            )
    ...
    def publish_manual_home(self, hands: tuple[str, ...] = ("left", "right")) -> None:
        ...
        label = "Both hands" if len(hands) > 1 else hands[0].capitalize()
        self.state.status = f"{label} HOME: returning"
    ...
    def collect_manual_home_result(self) -> None:
        ...
        try:
            ...
            if (not self.state.paused and self.ik_publisher is not None
                ...
                self.state.status = status
                for hand in self.manual_home_hand or ():
                    self.state.home_needs_sync.discard(hand)
                if self.state.parked_after_place_hand in (self.manual_home_hand or ()):
                    self.state.parked_after_place_hand = None
    ...
    def handle_key(self, key: int, bundle) -> bool:
        ...
        if key == KEY_PAUSE:
            ...
            if self.state.paused and (
                ...
                or is_pending(getattr(self, "place_future", None))
                or is_pending(self.manual_home_future)
    ...
    def handle_tablet_commands(self, bundle) -> None:
        """Execute queued browser commands in the main application loop."""
        ...
        while True:
            ...
            if self.tablet_bridge is not None:
                self.tablet_bridge.publish(
                    ...
                    base_target_text=(
                        format_base_target_text(self.state.base_targets)
                        if self.args.show_base_targets
                        and self.state.flowpose_overlay is not None
                        else ""
                    ),
                )
            self.render(bundle)
